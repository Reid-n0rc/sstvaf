...
{
    // Per-protocol slot length and symbol period. FT2 shares FT4's symbol layout but runs
    // at half the symbol period (double baud); everything downstream is derived from these.
    float slot_time, symbol_period;
    switch (cfg->protocol)
    {
